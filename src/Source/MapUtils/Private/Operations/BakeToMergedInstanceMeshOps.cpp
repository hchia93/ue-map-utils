...
#define LOCTEXT_NAMESPACE "BakeToMergedInstanceMeshOps"
...
namespace
{
    ...
    bool IsAcceptableSource(AActor* Actor)
    {
        ...
        return Actor->ActorHasTag(IsmBaked::Tag);
    }

    void HarvestFromComponent(AActor* OwningActor, UStaticMeshComponent* SMC, TArray<FInstanceEntry>& OutEntries, TArray<FBakeProfileSample>& OutSamples)
    {
...
FBakeMergedInstanceResult FBakeToMergedInstanceMeshOps::BakeToMergedInstanceMesh(const TArray<AActor*>& Actors)
{
    ...
    TArray<FBakeProfileSample> Samples;
    ...
    // MigrateStaticMeshComponentSettingsOps::AreGroupableSettingsEqual + Copy.
    {
        TMap<FName, int32> ProfileCounts;
        for (const FBakeProfileSample& S : Samples)
        {
    ...
    for (const FInstanceEntry& Entry : Entries)
    {
        const int32 Idx = Groups.IndexOfByPredicate([&](const FGroup& G)
        {
            return G.bReverseCulling == Entry.bReverseCulling
                && MigrateStaticMeshComponentSettingsOps::AreGroupableSettingsEqual(G.TemplateSmc, Entry.SourceSmc);
        });
        if (Idx == INDEX_NONE)
        {
            FGroup G;
...

    {
        ...
        for (const FGroup& Group : Groups)
        {
            ...
            MigrateStaticMeshComponentSettingsOps::Copy(Group.TemplateSmc, Ismc);
        ...
        IsmBaked::TagAndLabel(MergedActor, FallbackFolder);
