#include "Operations/MapUtilsSanitizeMapReferenceCommandlet.h"
...
UMapUtilsSanitizeMapReferenceCommandlet::UMapUtilsSanitizeMapReferenceCommandlet()
{
    IsClient = false;
    IsEditor = true;
    IsServer = false;
...
int32 UMapUtilsSanitizeMapReferenceCommandlet::Main(const FString& Params)
{
...
bool UMapUtilsSanitizeMapReferenceCommandlet::ParseOptions(const FString& Params, FOptions& OutOptions) const
{
    FString LevelsValue;
    ...
    if (!FParse::Value(*Params, TEXT("-report="), OutOptions.ReportPath, false) || OutOptions.ReportPath.IsEmpty())
    {
        ...
        OutOptions.ReportPath = FPaths::Combine(FPaths::ProjectDir(), TEXT("Intermediate"), TEXT("MapUtilsSanitizeMapReference"), FString::Printf(TEXT("%s.json"), *Stamp));
    }
...
bool UMapUtilsSanitizeMapReferenceCommandlet::ResolvePairs(const TArray<FReplacePair>& Pairs, TMap<UObject*, UObject*>& OutReplaceMap) const
{
...
bool UMapUtilsSanitizeMapReferenceCommandlet::ProcessLevel(const FString& LevelPath, const TMap<UObject*, UObject*>& ReplaceMap, bool bDryRun, FMapResult& OutResult) const
{
...
bool UMapUtilsSanitizeMapReferenceCommandlet::WriteReport(const FString& ReportPath, const TArray<FMapResult>& Results, bool bDryRun) const
{
