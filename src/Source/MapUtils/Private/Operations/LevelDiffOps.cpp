...
FActorChangeSummary FLevelDiffOps::GetActorChanges(AActor* Actor)
{
    ...
    TMap<FString, FObjectChange> Aggregated;
    ...
    for (int32 QueueIndex = 0; QueueIndex < ActiveCount; ++QueueIndex)
    {
        ...
        for (const TPair<FName, TSharedPtr<FTransactionObjectEvent>>& Pair : Diff.DiffMap)
        {
            ...
            FObjectChange& Change = Aggregated.FindOrAdd(ObjectPath);
    ...
    Result.Objects.Sort([](const FObjectChange& A, const FObjectChange& B)
    {
        return A.ObjectPath < B.ObjectPath;
    });

    for (FObjectChange& Change : Result.Objects)
    {
