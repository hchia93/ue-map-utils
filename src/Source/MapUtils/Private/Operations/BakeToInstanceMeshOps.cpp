...
#define LOCTEXT_NAMESPACE "BakeToInstanceMeshOps"
...
namespace
{
    AActor* SpawnIsmActorForSource(UWorld* World, ULevel* Level, AStaticMeshActor* Source, int32 LabelIndex)
    {
        ...
        MigrateStaticMeshComponentSettingsOps::Copy(Src, Ismc);
        ...
        IsmBaked::TagAndLabelWithIndex(Out, LabelIndex, Source->GetFolderPath());
...
FBakeInstanceResult BakeToInstanceMeshOps::BakeToInstanceMesh(const TArray<AStaticMeshActor*>& Actors)
{
    ...
    int32 NextLabelIdx = IsmBaked::PeekNextLabelIndex(World);
