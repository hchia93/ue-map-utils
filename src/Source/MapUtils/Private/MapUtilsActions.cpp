...
void FMapUtilsActions::AuditCurrentLevel()
{
    ...
    for (TActorIterator<AStaticMeshActor> It(World); It; ++It)
    {
        ...
        {
            TSharedRef<FTokenizedMessage> Message = Log.Error(FText::Format(
                LOCTEXT("NullStaticMesh", "StaticMesh is null on {0}"),
                FText::FromString(Actor->GetName())));
            Message->AddToken(FUObjectToken::Create(Actor));
    ...
    UE_LOG(LogMapUtils, Log, TEXT("AuditCurrentLevel: %d issue(s) found in %s"),
        IssueCount, *World->GetMapName());
}
...
void FMapUtilsActions::ConvertSelectedToBlockingVolume()
{
    ...
    if (Selected.IsEmpty())
    {
        Log.Warning(LOCTEXT("NoSelection",
            "No StaticMeshActor in current selection. Select one or more in the Outliner / viewport first."));
        Log.Open(EMessageSeverity::Info, true);
        return;
    }

    const FMapUtilsBlockingVolumeConvertResult Result =
        FMapUtilsBlockingVolumeOps::ConvertActorsToBlockingVolumes(Selected);

    if (Result.bSuccess)
    {
        Log.Info(FText::Format(
            LOCTEXT("ConvertSuccess",
                "Converted {0} actor(s) into {1} BlockingVolume(s) across {2} cluster(s). Ctrl+Z to undo."),
            Result.DestroyedActorNames.Num(), Result.CreatedVolumes.Num(), Result.ClusterCount));
    }
    else
    {
        Log.Error(LOCTEXT("ConvertFailed",
            "Convert to BlockingVolume failed. See Output Log for details."));
    }

    Log.Open(EMessageSeverity::Info, true);

    UE_LOG(LogMapUtils, Log,
        TEXT("ConvertSelectedToBlockingVolume: %d -> %d (success=%d)"),
        Result.DestroyedActorNames.Num(), Result.CreatedVolumes.Num(),
        Result.bSuccess ? 1 : 0);
}
...
void FMapUtilsActions::ExportStaticMeshContext()
{
    ...
    {
        Log.Info(FText::Format(
            LOCTEXT("ExportStaticMeshOK", "Exported {0} actor(s) to {1}"),
            Result.ItemCount, FText::FromString(Result.OutputPath)));
    }
    else
    {
...
void FMapUtilsActions::ExportCollisionContext()
{
    ...
    {
        Log.Info(FText::Format(
            LOCTEXT("ExportCollisionOK", "Exported {0} candidate(s) to {1}"),
            Result.ItemCount, FText::FromString(Result.OutputPath)));
    }
    else
    {
