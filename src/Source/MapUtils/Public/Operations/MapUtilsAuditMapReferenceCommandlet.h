...
#include "MapUtilsAuditMapReferenceCommandlet.generated.h"
 ...
 *
 * Usage:
 *   UnrealEditor-Cmd.exe Project.uproject -run=MapUtilsAuditMapReference
 ...
 *   -report=   Where to write the report JSON. Defaults under Intermediate/MapUtilsAuditMapReference/.
 *
 * Exit code: 0 = no broken refs, 2 = broken refs found (gate the sync commit),
 *            1 = bad args or report write failure.
 */
UCLASS()
class UMapUtilsAuditMapReferenceCommandlet : public UCommandlet
{
    GENERATED_BODY()

public:

    UMapUtilsAuditMapReferenceCommandlet();
